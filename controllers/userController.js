...
const getNotificationsList = asyncHandler(async (req, res) => {
  ...
  if (!userId) {
    console.error("[GetNotifications] missing userId on req.user");
  ...
  try {
    ...
    console.log("[GetNotifications] found:", notice?.length || 0);
    ...
    try {
      ...
      console.log("[GetNotifications] fallback found:", notice?.length || 0);
      return res.status(200).json(notice);
    } catch (err2) {
      console.error(
        "[GetNotifications] fallback error:",
        err2?.message || err2
      );
      return res
        .status(500)
        .json({ status: false, message: "Failed to retrieve notifications" });
    }
  }
});
...
const forgotPassword = asyncHandler(async (req, res) => {
  ...
  res
    .status(200)
    .json({ status: true, resetUrl, message: "Password reset token created" });
});
...
const resetPassword = asyncHandler(async (req, res) => {
  ...
  const tokenJWT = createJWT(res, user._id);
  user.password = undefined;

  res.status(200).json({ ...(user._doc || {}), token: tokenJWT });
});
